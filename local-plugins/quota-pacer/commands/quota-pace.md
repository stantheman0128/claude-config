...
- 起點寫 `active.json`（`u0` 含 scoped 和/或 `minutes`、`started`）→ 分塊做、每塊後跑 `eval_pace.py` 印進度 → WINDDOWN 收手、HARDSTOP 停並收尾（未完成呼叫 `handoff` skill 寫交接）→ 收尾刪 `active.json`。
- 用量自動從官方 endpoint 拉（STALE 自刷新），任何 harness 都能用；連自刷新都失敗（credentials 缺／斷網）且沒給時間上限才停下回報。
